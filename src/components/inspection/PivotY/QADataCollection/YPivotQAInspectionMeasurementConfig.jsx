...
const YPivotQAInspectionMeasurementConfig = ({
  ...
  // Helper to get measurement point name based on language
  const getMeasurementPointName = useCallback(
    (spec) => {
      if (currentLanguage === "ch") {
        // Chinese language: prioritize Chinese, fallback to English
        return (
          spec.MeasurementPointChiName || spec.MeasurementPointEngName || "N/A"
        );
      } else {
        // English language: prioritize English, fallback to Chinese
        return (
          spec.MeasurementPointEngName || spec.MeasurementPointChiName || "N/A"
        );
      }
    },
    [currentLanguage],
  ...
  // const getMeasurementPointName = useCallback(
  //   (spec) => {
  //     if (currentLanguage === "ch" && spec.MeasurementPointChiName) {
  //       return spec.MeasurementPointChiName;
  //     }
  //     return spec.MeasurementPointEngName;
  //   },
  //   [currentLanguage],
  // );
  ...
  const handleDeleteMeasurement = (index) => {
    if (
      !window.confirm(
        t("fincheckInspectionMeasurementConfig.confirm.deleteMessage"),
      )
    )
      return;
  ...
  if (!activeMoNo)
    return (
      <div className="p-8 text-center bg-white rounded-xl shadow">
        {t("fincheckInspectionMeasurementConfig.errors.noOrder")}
      </div>
    );
  if (measConfig === "No")
    return (
      <div className="p-8 text-center bg-white rounded-xl shadow">
        {t("fincheckInspectionMeasurementConfig.errors.notRequired")}
      </div>
    );
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h3 className="font-bold text-gray-800 dark:text-white flex items-center gap-2">
            ...
            {t("fincheckInspectionMeasurementConfig.header.measurement")}{" "}
            {displayLabel || `${measConfig} Wash`}
          </h3>
          <p className="text-xs text-gray-500 mt-1">
            {t("fincheckInspectionMeasurementConfig.header.order")}{" "}
            <span className="font-mono text-indigo-600 dark:text-indigo-400">
              {activeMoNo}
            </span>{" "}
            • {selectedSpecsList.length}{" "}
            {t("fincheckInspectionMeasurementConfig.header.criticalPoints")}
        ...
        <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-xl">
          ...
          >
            ...
            {t("fincheckInspectionMeasurementConfig.tabs.manual")}
          </button>
          <button
          ...
          >
            {t("fincheckInspectionMeasurementConfig.tabs.setupMeasure")}
          </button>
          <button
          ...
          >
            {t("fincheckInspectionMeasurementConfig.tabs.results")}{" "}
          ...
          >
            {t("fincheckInspectionMeasurementConfig.tabs.summary")}
          </button>
        </div>
      </div>
        ...
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-3 rounded-xl flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="text-sm font-bold text-green-800 dark:text-green-300 flex flex-wrap gap-1">
              <span className="mr-1">
                {t("fincheckInspectionMeasurementConfig.session.active")}
              </span>
              {activeGroup.lineName && (
                <span className="bg-white/50 px-1.5 rounded border border-green-200">
                  {t("fincheckInspectionMeasurementConfig.session.line")}{" "}
                  {activeGroup.lineName}
                </span>
              )}
              {activeGroup.tableName && (
                <span className="bg-white/50 px-1.5 rounded border border-green-200">
                  {t("fincheckInspectionMeasurementConfig.session.table")}{" "}
                  {activeGroup.tableName}
                </span>
              )}
              {activeGroup.colorName && (
                <span className="bg-white/50 px-1.5 rounded border border-green-200">
                  {t("fincheckInspectionMeasurementConfig.session.color")}{" "}
                  {activeGroup.colorName}
                </span>
              )}
            </div>
          ...
          {activeGroup.activeQC && (
            <div className="text-xs text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-800 px-2 py-1 rounded">
              {t("fincheckInspectionMeasurementConfig.session.qc")}{" "}
              {activeGroup.activeQC.eng_name}
            </div>
          )}
        </div>
      ) : (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-4 rounded-xl flex items-start gap-3">
          ...
          <div>
            <p className="font-bold text-amber-700 dark:text-amber-400">
              {t("fincheckInspectionMeasurementConfig.warnings.noContext")}
            </p>
            <p className="text-sm text-amber-600 dark:text-amber-500">
              <Trans
                i18nKey="fincheckInspectionMeasurementConfig.warnings.noContextDesc"
            ...
            >
              <div className="flex justify-between items-center mb-4">
                <h4 className="font-bold text-sm text-gray-700 dark:text-gray-300">
                  {t("fincheckInspectionMeasurementConfig.setup.title")}
                </h4>
                <button
                ...
                >
                  ...
                  {t("fincheckInspectionMeasurementConfig.setup.configure")}
                </button>
              </div>
              ...
              <div className="space-y-4">
                ...
                {measConfig === "Before" && kValuesList.length > 0 && (
                  <div>
                    <label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-1.5 flex items-center gap-2">
                      {t("fincheckInspectionMeasurementConfig.setup.kValue")}{" "}
                      {canSelectSizeAndK ? (
                      ...
                      {selectedKValue && (
                        <span className="text-purple-500 text-[10px] font-normal">
                          (
                          {t(
                            "fincheckInspectionMeasurementConfig.setup.selected",
                          )}{" "}
                          {selectedKValue})
                        </span>
                      )}
                    ...
                    >
                      <option value="">
                        {t("fincheckInspectionMeasurementConfig.setup.selectK")}
                      </option>
                      {kValuesList.map((k) => (
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-1.5 flex items-center gap-2">
                    {t("fincheckInspectionMeasurementConfig.setup.size")}{" "}
                    {canSelectSizeAndK ? (
                      <Unlock className="w-3 h-3 text-green-500" />
                    ) : (
                      <Lock className="w-3 h-3 text-gray-400" />
                        ...
                        >
                          ...
                          {status?.isComplete && (
                            <span className="text-[10px] bg-green-500 text-white px-1.5 py-0.5 rounded font-bold">
                              {t(
                                "fincheckInspectionMeasurementConfig.setup.done",
                              )}
                            </span>
                          )}
                        </button>
                ...
                >
                  ...
                  {t(
                    "fincheckInspectionMeasurementConfig.setup.startMeasuring",
                  )}
                </button>
              </div>
            </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 h-full max-h-[500px] flex flex-col">
              <h4 className="font-bold text-sm text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
                ...
                {t("fincheckInspectionMeasurementConfig.preview.title")} (
