...
export const MeasurementStatsCards = ({ stats, compact = false }) => {
  ...
  const cards = [
    {
      label: t("fincheckInspectionMeasurementSummary.stats.totalPoints"),
      value: stats.totalPoints,
      color: "blue",
      icon: Hash,
    },
    {
      label: t("fincheckInspectionMeasurementSummary.stats.failPoints"),
      value: stats.failPoints,
      color: "red",
      icon: XCircle,
    },
    {
      label: t("fincheckInspectionMeasurementSummary.stats.pointPassRate"),
      value: `${stats.pointPassRate}%`,
      color: "green",
      icon: Percent,
    },
    {
      label: t("fincheckInspectionMeasurementSummary.stats.totalPcs"),
      value: stats.totalPcs,
      color: "indigo",
      icon: Target,
    },
    {
      label: t("fincheckInspectionMeasurementSummary.stats.failPcs"),
      value: stats.failPcs,
      color: "orange",
      icon: TrendingDown,
    },
    {
      label: t("fincheckInspectionMeasurementSummary.stats.pcsPassRate"),
...
export const MeasurementLegend = ({ compact = false }) => {
  // 1. Initialize the hook inside the component body
  const { t } = useTranslation();

  // 2. Return the JSX explicitly
  return (
    <div
    ...
    >
      ...
      >
        {t("fincheckInspectionMeasurementSummary.legend.label")}
      </span>
      <div className="flex items-center gap-1">
        ...
        >
          {t("fincheckInspectionMeasurementSummary.legend.all")}
        </span>
      </div>
      <div className="flex items-center gap-1">
        <span
        ...
        <span
          className={`${compact ? "text-[9px]" : "text-[10px]"} text-gray-600`}
        >
          {t("fincheckInspectionMeasurementSummary.legend.critical")}
        </span>
      </div>
      <div className="flex items-center gap-1">
        <Star
        ...
        <span
          className={`${compact ? "text-[9px]" : "text-[10px]"} text-gray-600`}
        >
          {t("fincheckInspectionMeasurementSummary.legend.criticalPt")}
        </span>
      </div>
      <div className="flex items-center gap-1">
        <div
        ...
        <span
          className={`${compact ? "text-[9px]" : "text-[10px]"} text-gray-600`}
        >
          {t("fincheckInspectionMeasurementSummary.legend.withinTol")}
        </span>
      </div>
      <div className="flex items-center gap-1">
        <div
        ...
        <span
          className={`${compact ? "text-[9px]" : "text-[10px]"} text-gray-600`}
        >
          {t("fincheckInspectionMeasurementSummary.legend.outOfTol")}
        </span>
      </div>
    </div>
  );
...
export const MeasurementSummaryTable = ({
  ...
  // Helper to get measurement point name based on language (with fallback)
  const getMeasurementPointName = useCallback(
    (spec) => {
      // Check for "ch" (from your custom event) or standard "zh" (from i18n)
      if (currentLanguage === "ch" || currentLanguage.startsWith("zh")) {
        // Chinese language: prioritize Chinese, fallback to English
        // Note: Added spec.MeasurementPointName as an extra fallback in case your API uses that key
        return (
        ...
        // English language: prioritize English, fallback to Chinese
        return (
          spec.MeasurementPointEngName ||
          spec.MeasurementPointChiName ||
          spec.MeasurementPointName ||
  ...
  if (
    ...
    return (
      <div className="text-center py-6 text-gray-400 text-xs italic">
        ...
        {t("fincheckInspectionMeasurementSummary.table.noData")}
      </div>
    );
  }
          ...
          <tr className="bg-gray-100 dark:bg-gray-800">
            ...
            >
              <div className="flex items-center gap-1">
                ...
                />
                <span>
                  {t("fincheckInspectionMeasurementSummary.table.point")}
                </span>
              </div>
            </th>
            ...
            >
              <span className="text-red-600 dark:text-red-400 font-bold">
                {t("fincheckInspectionMeasurementSummary.table.tolMinus")}
              </span>
            </th>
            <th
              rowSpan={2}
            ...
            >
              <span className="text-green-600 dark:text-green-400 font-bold">
                {t("fincheckInspectionMeasurementSummary.table.tolPlus")}
              </span>
            </th>
              ...
              <React.Fragment key={sIdx}>
                ...
                {sizeData.columns.length > 0 && (
                  ...
                  >
                    ...
                    >
                      {t("fincheckInspectionMeasurementSummary.table.spec")}
                    </span>
                  </th>
                )}
          ...
          <tr className="bg-gradient-to-r from-gray-700 to-gray-800 text-white">
            <th className="border border-gray-600 px-3 py-2 text-left font-bold min-w-[140px]">
              <div className="flex items-center gap-1.5">
                ...
                {t("fincheckInspectionMeasurementSummary.table.config")}
              </div>
            </th>
            ...
            <th className="border border-gray-600 px-3 py-2 text-center font-bold min-w-[80px] bg-indigo-700">
              {t("fincheckInspectionMeasurementSummary.table.result")}
            </th>
          </tr>
        </thead>
        <tbody>
          {allGroups.map((group, gIdx) => {
            const configLabel =
              [
                ...
                .filter(Boolean)
                .join(" / ") ||
              t("fincheckInspectionMeasurementSummary.group.general");
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {isPass
                          ? t(
                              "fincheckInspectionMeasurementSummary.status.pass",
                            )
                          : t(
                              "fincheckInspectionMeasurementSummary.status.fail",
                            )}
                      </span>
                    </td>
                  );
...
const YPivotQAInspectionMeasurementSummary = ({
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 px-4">
        ...
        <h3 className="text-base font-bold text-gray-600 dark:text-gray-400 mb-1">
          {t("fincheckInspectionMeasurementSummary.empty.title")}
        </h3>
        <p className="text-xs text-gray-500 text-center max-w-md">
          {t("fincheckInspectionMeasurementSummary.empty.description")}
        </p>
      </div>
    );
  }
  ...
  const renderGroupSection = (group, isActive) => {
    const headerParts = [
      group.lineName
        ? `${t("fincheckInspectionMeasurementSummary.group.line")} ${group.lineName}`
        : null,
      group.tableName
        ? `${t("fincheckInspectionMeasurementSummary.group.table")} ${group.tableName}`
        : null,
      group.colorName
        ? `${t("fincheckInspectionMeasurementSummary.group.color")} ${group.colorName}`
        : null,
    ].filter(Boolean);
    const headerLabel =
      headerParts.length > 0
        ? headerParts.join(" / ")
        : t("fincheckInspectionMeasurementSummary.group.session");
          ...
          <div className="flex-1">
            ...
            {group.qcUser && (
              <p className="text-[10px] text-gray-500">
                {t("fincheckInspectionMeasurementSummary.group.qc")}{" "}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="bg-gradient-to-r from-indigo-500 to-purple-600 px-3 py-2">
            <h3 className="text-white font-bold text-sm flex items-center gap-1.5">
              ...
              {t("fincheckInspectionMeasurementSummary.group.title")}
            </h3>
          </div>
